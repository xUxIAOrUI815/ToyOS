...
step1: 定义内核结束的标志，确定内存边界，即知道内核代码结束在哪里，剩下的内存才能用来分配。
step2: 创建内存管理器，创建一个简单的“栈式分配器”
    分配 alloc 把栈顶的空闲页弹出
    回收 dealloc 把不要的页面压回栈顶


### 物理页分配器
支持回收再利用和线性取页，同时对于取出的页做清0处理
ppn 第几号页
recycled_pages[] 是一个 LIFO 栈，保存此前 frame_free() 交回来的页号。
栈顶指针 recycled_ptr 大于 0 说明有“旧页”可复用，弹出栈顶页号即可；
时间复杂度 O(1)，而且刚释放的页很可能还在 cache 里，速度最快。
没有可回收的页时，就从 线性分配区 切一页：
– 把当前空闲区起始地址换算成页号；
– 再把起始指针 向后推 4 KB（下次就从下一页开始）。

## 虚拟内存与页表
有了物理内存分配，但是程序直接访问物理地址。
Shell需要fork，父子进程需要完全相同的内存布局，但是数据需要隔离。

解决方案：RISC-V 的SV39分页机制
建立一张映射表

暂时不开启 MMU（那会导致所有地址瞬间失效），而是先在内存里把页表建立起来，打印出来看看对不对。

## 虚拟内存
恒等映射
我们需要在页表里把内核所在的物理地址 0x80200000 映射到虚拟地址 0x80200000（虚实地址相等）。这样开启分页后，内核代码还能继续正常跑。

## 独立的进程地址空间
给每个任务分配独立的页表
实现用户空间映射 把User App 代码映射到虚拟地址 0x0 或 0x10000
实现 satp 切换：当任务切换时，不仅切换栈sp，还要切换页表 satp
